...
- use Map data structure as a cache
...
function memoizeInFlightFetchCalls(fn, ttl = 5000) {
  let cache = new Map();
  const memoize = (...args) => {
...
const fetchData = async (user) => {
  await delay(1000)
  return { name: user.firstName, city: user.city, text: `I am a ${user.job} from ${user.city}`}
}

const memoizedFetch = memoizeInFlightFetchCalls(fetchData);
